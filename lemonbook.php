...
function lemonbook_load_more_scripts() {
	...
	wp_register_script( 'lemonbook_loadmore', plugin_dir_url( __FILE__ ) . 'assets/js/loadmore.js', array('jquery') );
	...
	wp_localize_script( 'lemonbook_loadmore', 'lemonbook_loadmore_params', array(
 	...
 	wp_enqueue_script( 'lemonbook_loadmore' );
}
 
add_action( 'wp_enqueue_scripts', 'lemonbook_load_more_scripts' );
 
 
add_action('wp_ajax_loadmore', 'lemonbook_shortcode_handler_function'); 
add_action('wp_ajax_nopriv_loadmore', 'lemonbook_shortcode_handler_function');
